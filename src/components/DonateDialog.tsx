...
const useStyles = makeStyles({
  donateMethod: { display: 'flex', flexDirection: 'column', gap: '1rem', marginTop: '1rem' },
  qrcodeImage: { width: '100%', maxWidth: '400px', height: 'auto', display: 'block', margin: '0 auto' },
});
  ...
  const renderContent = () => {
    ...
    if (selectedRecipient === 'ntkrnl') {
      return (
        <>
          <DonateOptionCard 
            icon={<Payment24Regular />} title="爱发电" description="微信、支付宝均可"
            onClick={() => window.open('https://afdian.com/a/ntkrnl32', '_blank')}
          />
          <DonateOptionCard 
            icon={<Payment24Regular />} title="支付宝" description="点击查看二维码"
            onClick={() => { setQrcodeImage('/alipay.png'); setImageLoading(true); }}
          />
          <DonateOptionCard 
            icon={<Image24Regular />} title="微信支付" description="点击查看二维码"
            onClick={() => { setQrcodeImage('/wechat.png'); setImageLoading(true); }}
          />
          <Button appearance="subtle" onClick={() => setSelectedRecipient(null)}>返回</Button>
        </>
      );
    ...
    if (selectedRecipient === 'nicrozoft') {
      return (
        <>
          <DonateOptionCard 
            icon={<Heart24Regular />} title="爱发电" description="支持 Nicrozoft"
            onClick={() => window.open('https://afdian.com/a/nicrozoft', '_blank')}
          />
          <DonateOptionCard 
            icon={<Payment24Regular />} title="B 站充电" description="前往 Bilibili"
            onClick={() => window.open('https://space.bilibili.com/3546641434937345', '_blank')}
          />
          <Button appearance="subtle" onClick={() => setSelectedRecipient(null)}>返回</Button>
        </>
      );
          ...
          <DialogBody>
            <DialogTitle>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Button appearance="subtle" icon={<Dismiss24Regular />} onClick={closeDialog} />
              </div>
            </DialogTitle>
            <DialogContent>
        ...
        <DialogSurface>
          <DialogBody>
            <DialogTitle>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span>付款二维码</span>
                <Button appearance="subtle" icon={<Dismiss24Regular />} onClick={() => setQrcodeImage(null)} />
              </div>
            </DialogTitle>
            <DialogContent>
